...
func (t *node) handleDeprecated(method method, pattern string, handler http.Handler) error {
...
func (t *node) match(method method, path string, ctx *Context) (http.Handler, error) {
	...
	for i := 0; i < len(path); {
		...
		case v > t.v:
			fmt.Println("GT", string(v), string(t.v))
			t = t.gt

		case v < t.v:
			fmt.Println("LT", string(v), string(t.v))
			t = t.lt
		...
		default:
			fmt.Println("EQ", string(v), string(t.v))
			t = t.eq
			i++
		}
	}
